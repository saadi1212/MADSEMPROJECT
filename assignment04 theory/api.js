...
// ===== RapidAPI Setup (Unsplash) =====
const api = axios.create({
  baseURL: "https://unsplash-api.p.rapidapi.com",
  headers: {
    "X-RapidAPI-Key": "YOUR_RAPID_API_KEY",
    "X-RapidAPI-Host": "unsplash-api.p.rapidapi.com",
  },
});
  ...
  const fetchPhotographers = async () => {
    try {
      ...
      const formattedData = response.data.results.map((item, index) => ({
        id: item.id,
        name: item.user.name || "Professional Photographer",
        price: (12000 + index * 1000).toString(),
        img: item.urls.small,
      }));
